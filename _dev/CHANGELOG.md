...
* Fix the "Productos" nav dropdown's product thumbnail images looking blurry - `Nav_Walker::start_el()` (`inc/includes/nav-walker.php`) was requesting core's 150x150 `'thumbnail'` size for a box that renders at ~280x210 (`.dropdown-item-thumbnail`, `_nav-desktop.scss`), an upscale of 2-4x depending on screen density. New `nav-dropdown-thumbnail` image size (560x420, hard-cropped, registered in `build_theme_support()`, `inc/includes/theme-setup.php`) matches that box at 2x for retina. Existing product images need a thumbnail regenerate (e.g. `wp media regenerate --image_size=nav-dropdown-thumbnail`) to pick up the new crop - newly uploaded/edited product images get it automatically

* Add example-value placeholders to the checkout form's billing/order inputs (`checkout_add_field_placeholders()`, new function in `inc/hooks/checkout.php`, hooked to `woocommerce_checkout_fields` alongside the existing `checkout_add_field_classes()`) - name, email, phone, address, city, postcode and order notes all previously rendered with no placeholder text. Also gave the custom `billing_neighborhood`/`shipping_neighborhood` field (`checkout_add_neighborhood_field()`) its own placeholder. Select fields (`billing_country`, `billing_state`) are left out since core's field template ignores placeholders on those
