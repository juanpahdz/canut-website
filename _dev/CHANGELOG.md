...
* Add an animated green glow to the front page hero CTA ("Comprar ahora"): a conic-gradient ring continuously rotates around the button's border only (two-layer `background-clip: padding-box, border-box` trick, animated via a registered `@property` angle so the browser can interpolate it smoothly instead of jumping) - the button's fill is untouched, keeping `button-canut-ghost`'s existing translucent-white hover tint rather than adding a new fill effect (`home-hero-cta` in `template-parts/front-page/hero.php`, `_front-page.scss`)
